...
const FeedItem = ({ item }) => {
  ...
  useEffect(() => {
    const fetchUserProfile = async () => {
      try {
        const userDocRef = doc(FIREBASE_DB, 'users', item.userId);
        ...
        if (userDocSnap.exists()) {
          ...
          setUserProfile({
            ...
            profileImage: userData.profileImage || 'defaultImageUrl',
          });
        }
      } catch (error) {
  ...
  useEffect(() => {
    const unsubscribeLikes = onSnapshot(doc(FIREBASE_DB, 'mediaPosts', item.id), (docSnap) => {
      if (docSnap.exists()) {
        ...
        setHasLiked(currentUser && likedBy.includes(currentUser.uid));
      }
    });
    ...
    const unsubscribeComments = onSnapshot(doc(FIREBASE_DB, 'mediaPosts', item.id, 'comments', 'totalCommentsCount'), (docSnap) => {
      if (docSnap.exists()) {
        ...
        setCommentCount(commentsData.totalCommentsCount || 0); // Use totalCommentsCount to set comment count
      }
    });
